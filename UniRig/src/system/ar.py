        ...
        for (id, detokenize_output) in enumerate(detokenize_output_list):
            ...
            if self.export_npz is not None:
                if not self.user_mode:
                    ...
                    # In user_mode, save NPZ to npz_dir so skinning step can find it
                    # paths[id] is already npz_dir/basename
                    npz_path = os.path.join(paths[id], f"{self.export_npz}.npz")
                    print(f"Saving NPZ (user_mode): {npz_path}")
